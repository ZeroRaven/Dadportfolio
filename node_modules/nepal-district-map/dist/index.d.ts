...
import { DistrictDataMap, Province } from './data.js';
export { DISTRICTS, DISTRICT_NAMES, DISTRICT_PROVINCE, DistrictData, DistrictPath, PROVINCES, PROVINCE_COLORS, PROVINCE_NAMES, ProvinceInfo } from './data.js';

type TooltipPosition = "top-left" | "top-right" | "bottom-left" | "bottom-right" | "follow-cursor";
interface NepalMapProps {
    ...
    provinceColors?: Partial<Record<Province, {
        fill: string;
        stroke: string;
    }>>;
...
declare function NepalMapLegend({ mode, items: customItems, direction, swatchShape, swatchSize, fontSize, labelColor, selectedProvince, onProvinceClick, className, style, renderItem, }: NepalMapLegendProps): react_jsx_runtime.JSX.Element;

/**
 * Get all district names belonging to a province.
 */
declare function getDistrictsByProvince(province: Province): string[];
/**
 * Get the province a district belongs to.
 * Returns undefined if the district name is not found.
 */
declare function getProvinceByDistrict(districtName: string): Province | undefined;
/**
 * Get summary stats from a DistrictDataMap.
 * Useful for building legends and info panels.
 */
declare function getDataStats(data: DistrictDataMap): {
    min: number;
    max: number;
    count: number;
    total: number;
 ...
 */
declare function createColorScale(min: number, max: number, fromColor: string, toColor: string): (value: number) => string;
/**
 ...
 */
declare function createMultiColorScale(min: number, max: number, colors: string[]): (value: number) => string;
/**
 * Get the total number of districts.
 */
declare function getTotalDistricts(): number;
/**
 * Get province-wise summary from data.
 */
declare function getProvinceSummary(data: DistrictDataMap): Array<{
    province: Province;
    color: string;
...
export { DistrictDataMap, type LegendItem, NepalMap, NepalMapLegend, type NepalMapLegendProps, type NepalMapProps, Province, type TooltipPosition, createColorScale, createMultiColorScale, getDataStats, getDistrictsByProvince, getProvinceByDistrict, getProvinceSummary, getTotalDistricts };
